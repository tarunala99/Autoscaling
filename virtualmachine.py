...
def demo():
    ...
    storage_account_name=list[0]
    group_name_for_new_vm = "Project24"
    


    image_uri = "https://" + storage_account_name + ".blob.core.windows.net/system/Microsoft.Compute/Images/vhds/lgp21f16v3-osDisk.7f5686b8-d969-4bb7-ab57-2e079071e923.vhd"
    vm_name = pse_uniqname + "vm"    # specify your vm name (should be public unique)
    load=vm_name+".eastus.cloudapp.azure.com"
    api.create_vm_from_ami(storage_name="projecttarunala", vm_name=str(vm_name), group_name=str(group_name_for_new_vm), image_uri=str(image_uri), machine_size="Standard_D1")
    print "Load generator launched"
    ...
    image_uri = "https://" + str(storage_account_name) + ".blob.core.windows.net/system/Microsoft.Compute/Images/vhds/cc15619p21dcv5-osDisk.e27faca3-f177-40ea-a740-9a1838326ae6.vhd"
    vm_name = pse_uniqname + "vm"    # specify your vm name (should be public unique)
    ...
    data=vm_name+".eastus.cloudapp.azure.com"
    print load
    time.sleep(50)
    ...
    while(count<=2):
        ...
        data=vm_name+".eastus.cloudapp.azure.com"
        time.sleep(50)
        swq=requests.post("http://"+str(load)+"/test/horizontal/add?dns="+str(data))
        print swq
        swq=requests.post("http://"+str(load)+"/test/horizontal/add?dns="+str(data))
        print swq
        swq=requests.post("http://"+str(load)+"/test/horizontal/add?dns="+str(data))
    ...
    list=[]
    while(count<=2):
        group_name_for_new_vm = "Project25"
        time_stamp = int(time.time()) % 10000
        rand_stamp = random.randint(1, 1000)
        pse_uniqname = "cc"+str(time_stamp)+str(rand_stamp)
        image_uri = "https://" + str(storage_account_name) + ".blob.core.windows.net/system/Microsoft.Compute/Images/vhds/cc15619p21dcv5-osDisk.e27faca3-f177-40ea-a740-9a1838326ae6.vhd"
